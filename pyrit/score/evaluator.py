...
from typing import Optional, Literal, List, Mapping, Any
...
class Evaluator(Scorer):
    ...
    def __init__(
        ...
        self.scorer_type: ScoreType = scorer_type  
        self.scorer_role = scorer_role or ""
        ...
        if evaluator_question is None:
            raise ValueError("evaluator_question cannot be None.")

        if isinstance(evaluator_question, EvaluatorQuestion):
            question_data: Mapping[str, Any] = {
                "category": evaluator_question.category,
                "evaluation_criteria": evaluator_question.evaluation_criteria,
                "metadata": evaluator_question.metadata,
            }
        elif isinstance(evaluator_question, Mapping):
            question_data = dict(evaluator_question)
        else:
            raise ValueError("evaluator_question must be an EvaluatorQuestion or a mapping.")
        ...
        self._score_category = question_data["category"]
    ...
    def validate(self, message_piece: MessagePiece, *, task: Optional[str] = None):
        pass
    ...
    async def _score_piece_async(
        ...
        """
        self.validate(message_piece, task=objective)
        ...
        unvalidated_score: UnvalidatedScore = await self._score_value_with_llm(
            ...
            request_prompt=message_piece.original_value,
        ...
        expected_output = (
            ...
            else message_piece.expected_output
        )
        ...
        score = Score(
            ...
            message_piece_id=unvalidated_score.message_piece_id,
